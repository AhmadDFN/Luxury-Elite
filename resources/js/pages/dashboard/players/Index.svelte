...
<div class="max-w-7xl mx-auto space-y-8 py-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div class="space-y-2">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#dca34f]/10 border border-[#dca34f]/30 text-[#dca34f] text-xs font-bold tracking-widest uppercase mb-2">
                <span class="w-2 h-2 rounded-full bg-[#dca34f] animate-pulse"></span>
                Official Team
            </div>
            <h1 class="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#dca34f] to-[#f4d49a] tracking-tight">
                Luxury Elite Roster
            </h1>
            <p class="text-gray-400 text-sm md:text-base max-w-lg">
                Manage your elite players, structure team divisions, and command the ultimate competitive roster.
            </p>
        </div>

        <div class="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            <div class="relative w-full sm:w-72 group">
                <div class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-500 group-focus-within:text-[#dca34f] transition-colors duration-300">
                    {@html icons.search}
                </div>
                <input
                    type="text"
            ...
            >
                ...
                <div class="absolute inset-0 h-full w-full rounded-2xl opacity-0 group-hover:opacity-20 bg-white transition-opacity duration-300"></div>
            </Link>
        </div>
    </div>

    <div class="relative group">
        <div class="absolute -inset-0.5 bg-gradient-to-b from-[#dca34f]/20 to-transparent rounded-[1.25rem] blur opacity-30 group-hover:opacity-50 transition duration-500"></div>
        
        <div class="relative bg-[#1a1f33] rounded-2xl shadow-2xl border border-[#2d344f] overflow-hidden backdrop-blur-sm">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#111526]/80 text-gray-400 text-xs uppercase tracking-widest border-b border-[#2d344f]">
                            ...
                            <th class="px-8 py-5 font-bold text-right">Command</th>
                        </tr>
                    </thead>
                        ...
                        {#if players.length === 0}
                            <tr>
                                <td colspan="5" class="px-6 py-24 text-center">
                                    <div class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-[#111526] border border-dashed border-[#dca34f]/30 text-[#dca34f] mb-5 shadow-[0_0_15px_rgba(220,163,79,0.1)]">
                                        {@html icons.add} </div>
                                    <h3 class="text-xl font-bold text-gray-100 mb-2">
                                        Roster is Currently Empty
                                    </h3>
                                    <p class="text-gray-500 max-w-sm mx-auto">
                                        No elite players have been recruited yet. Add your first player to begin your path to victory.
                                    </p>
                                </td>
                            </tr>
                        {:else}
                            {#each players as player}
                                <tr class="hover:bg-gradient-to-r hover:from-[#dca34f]/5 hover:to-transparent transition-all duration-300 group/row">
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div class="relative">
                                                {#if player.photo}
                                                    ...
                                                    />
                                                {:else}
                                                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-[#111526] to-[#1f2642] ring-2 ring-[#dca34f]/30 group-hover/row:ring-[#dca34f] flex items-center justify-center text-[#dca34f] text-lg font-bold shadow-lg transition-all duration-300">
                                                        {player.nickname.charAt(0).toUpperCase()}
                                                    </div>
                                                {/if}
                                                <div class="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-emerald-500 border-2 border-[#1a1f33] rounded-full"></div>
                                            </div>

                                            <div>
                                                <div class="text-base font-bold text-gray-100 group-hover/row:text-[#dca34f] transition-colors duration-300">
                                                    {player.nickname}
                                                </div>
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/40 border border-white/5 shadow-inner">
                                            ...
                                            <span class="font-mono text-sm font-semibold text-gray-300 tracking-wider">
                                                {player.uid_game || 'UNREGISTERED'}
                                            </span>
                                        </div>
                                    </td>

                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 backdrop-blur-sm">
                                            {player.division}
                                        </span>
                                    </td>

                                    <td class="px-6 py-5 whitespace-nowrap">
                                        <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider bg-[#dca34f]/10 text-[#dca34f] border border-[#dca34f]/20 shadow-[0_0_10px_rgba(220,163,79,0.1)] backdrop-blur-sm">
                                            {player.role}
                                        </span>
                                    </td>

                                    <td class="px-8 py-5 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end gap-2 opacity-50 group-hover/row:opacity-100 transition-opacity duration-300">
                                            <Link
